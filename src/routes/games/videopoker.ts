...
import { dealVideoPokerDeck, evaluateHand, PAY_TABLE } from "../../games/videopoker";
...
videoPokerRouter.post("/deal", requireAuth, requireApproved, async (req: AuthedRequest, res) => {
  ...
  if (videoPokerRounds.has(userId)) return res.status(409).json({ error: "You already have an active Video Poker hand — draw first" });
...
const drawSchema = z.object({ hold: z.array(z.boolean()).length(5) });
...
videoPokerRouter.post("/draw", requireAuth, requireApproved, async (req: AuthedRequest, res) => {
  ...
  let drawCursor = 5;
  const finalHand = round.hand.map((card, i) => hold[i] ? card : round.deck[drawCursor++]);
  ...
  res.json({ hand: finalHand, handRank, multiplier, payout, balance: settled.balance, level: settled.level, xp: settled.xp, leveledUp: settled.leveledUp, betId: settled.betId });
});
...
videoPokerRouter.get("/active", requireAuth, requireApproved, async (req: AuthedRequest, res) => {
  ...
  res.json({ hand: round.hand, bet: round.bet, fairness: { serverSeedHash: hashServerSeed(round.serverSeed), clientSeed: round.clientSeed, nonce: round.nonce } });
});

async function settleVideoPoker(userId: string, round: VideoPokerActiveRound, finalHand: VideoPokerActiveRound["hand"], handRank: string, multiplier: number, payout: number) {
  return prisma.$transaction(async (tx) => {
    ...
    const bet = await tx.bet.create({
      data: {
        userId, game: "videopoker", amount: round.bet, payout, multiplier,
        result: payout > 0 ? "win" : "loss",
        state: JSON.stringify({ initialHand: round.hand, finalHand, handRank }),
