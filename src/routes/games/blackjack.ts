...
import {
  freshShoe, startRound, dealCard, canSplit, canDouble, handValue, isBust, playDealerHand, settleHands, BlackjackState, BlackjackHand,
...
blackjackRouter.post("/start", requireAuth, requireApproved, async (req: AuthedRequest, res) => {
  ...
  try {
    const round = await prisma.$transaction(async (tx) => {
      ...
      const active: BlackjackActiveRound = { state, serverSeed: user.serverSeed, clientSeed: user.clientSeed, nonce: user.nonce, startedAt: Date.now() };
      return active;
    });
...
blackjackRouter.post("/action", requireAuth, requireApproved, async (req: AuthedRequest, res) => {
  ...
  try {
    switch (parsed.data.action) {
      case "hit": await applyHit(hand, state); break;
      case "stand": hand.status = "stood"; break;
      case "double": await applyDouble(userId, hand, state); break;
      case "split": await applySplit(userId, hand, state); break;
      case "surrender":
        if (hand.cards.length !== 2 || state.hands.length > 1) return res.status(400).json({ error: "Surrender is only available on your initial two-card hand" });
        hand.status = "surrendered"; break;
      case "insurance":
        if (!state.insuranceOffered || state.insuranceTaken || state.hands.length > 1 || hand.cards.length !== 2) return res.status(400).json({ error: "Insurance is not available right now" });
        await applyInsurance(userId, state); break;
    }
  } catch (err) {
    if (err instanceof InsufficientFundsError) return res.status(400).json({ error: "Insufficient balance" });
    if (err instanceof BadAction) return res.status(400).json({ error: (err as BadAction).message });
    throw err;
  }
...
async function applyDouble(userId: string, hand: BlackjackHand, state: BlackjackState) {
  ...
  hand.bet *= 2; hand.doubled = true; hand.cards.push(dealCard(state));
...
async function applySplit(userId: string, hand: BlackjackHand, state: BlackjackState) {
  ...
  hand.cards = [first];
  hand.cards.push(dealCard(state)); newHand.cards.push(dealCard(state));
  if (first.rank === "A") { hand.status = "stood"; newHand.status = "stood"; }
  else if (handValue(hand.cards).total === 21) hand.status = "stood";
...
async function applyInsurance(userId: string, state: BlackjackState) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (handValue(state.dealer).total === 21 && state.dealer.length === 2) await applyLedgerEntry(tx, userId, "payout", insuranceCost * 3, "insurance");
  });
  state.insuranceTaken = true;
}
class BadAction extends Error {}
function advanceIfNeeded(state: BlackjackState) {
  while (state.activeHand < state.hands.length && state.hands[state.activeHand].status !== "playing") state.activeHand += 1;
  if (state.activeHand >= state.hands.length) playDealerHand(state);
}
async function finishRound(userId: string, round: BlackjackActiveRound) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (totalPayout > 0) user = await applyLedgerEntry(tx, userId, "payout", totalPayout, undefined);
    const gainedXp = xpForWager(totalWagered);
    const newXp = user.xp + gainedXp; const newLevel = levelFromXp(newXp); const leveledUp = newLevel > user.level;
    const levelBonus = leveledUp ? newLevel * 500 : 0;
    user = await tx.user.update({ where: { id: userId }, data: { xp: newXp, level: newLevel, ...(levelBonus > 0 ? { balance: { increment: levelBonus } } : {}) } });
    if (levelBonus > 0) await tx.transaction.create({ data: { userId, type: "levelup_bonus", amount: levelBonus, balance: user.balance, reference: `level_${newLevel}` } });
    const overallResult = totalPayout > totalWagered ? "win" : "loss";
    const bet = await tx.bet.create({ data: { userId, game: "blackjack", amount: totalWagered, payout: totalPayout, multiplier: totalWagered > 0 ? Number((totalPayout / totalWagered).toFixed(4)) : 0, result: overallResult, state: JSON.stringify({ dealer: round.state.dealer, hands: round.state.hands, settlements }), clientSeed: round.clientSeed, serverSeed: round.serverSeed, nonce: round.nonce } });
    return { betId: bet.id, payout: totalPayout, balance: user.balance, level: user.level, xp: user.xp, leveledUp, table: publicTable(round, true), settlements };
  });
}
function publicTable(round: BlackjackActiveRound, revealHole: boolean) {
  ...
  return {
    status: state.status, activeHand: state.activeHand, insuranceOffered: state.insuranceOffered, insuranceTaken: state.insuranceTaken,
    dealer: { cards: dealerCards, value: dealerValue, holeHidden: !(revealHole || state.status !== "player_turn") },
    hands: state.hands.map((hand) => ({ cards: hand.cards, bet: hand.bet, status: hand.status, doubled: hand.doubled, value: handValue(hand.cards), canHit: hand.status === "playing", canStand: hand.status === "playing", canDouble: hand.status === "playing" && canDouble(hand), canSplit: hand.status === "playing" && canSplit(hand) })),
