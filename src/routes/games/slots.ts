...
slotsRouter.post("/spin", requireAuth, requireApproved, async (req: AuthedRequest, res) => {
  ...
  try {
    const placed = await placeBet(req.userId!, "slots", totalStake, (seeds) => {
      ...
      return { payout, multiplier: totalStake > 0 ? Number((payout / totalStake).toFixed(4)) : 0, result: payout >= totalStake ? "win" : "loss", state: { grid: spin.grid, lineBet, lines, lineWins: spin.lineWins, scatterCount: spin.scatterCount, scatterPayout: spin.scatterPayout, freeSpinsAwarded: spin.freeSpinsAwarded } };
    });
    res.json({ bet: placed.bet, balance: placed.balance, level: placed.level, xp: placed.xp, leveledUp: placed.leveledUp, result: placed.resolution });
  } catch (err) {
    if (err instanceof InsufficientFundsError) return res.status(400).json({ error: "Insufficient balance" });
    if (err instanceof BadBetInputError) return res.status(400).json({ error: (err as BadBetInputError).message });
    throw err;
  }
});
...
slotsRouter.get("/info", (_req, res) => {
  res.json({
    reels: 5, rows: 3, maxLines: MAX_LINES,
    symbols: ["wild", "scatter", "crown", "gem", "bell", "clover", "horseshoe", "ace", "king", "queen"],
    rules: ["Wilds substitute for any paying symbol and double the line win for each wild in the combo.", "Scatters pay on any position: 3 = 5x, 4 = 25x, 5 = 100x total bet, and 3+ award 10 free spins.", "Line wins pay left-to-right starting from reel 1; only the highest combo per line counts."],
  });
});
