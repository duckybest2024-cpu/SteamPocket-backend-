const VideoPokerGame = (() => {
  const HAND_NAMES = {
    ...
    two_pair:"Two Pair",jacks_or_better:"Jacks or Better",high_card:"No Win",
  };
  const PAY_TABLE = [
  ...
  function render(container, accountState) {
      ...
      <div class="game-panel"><div class="game-layout">
        <div class="bet-panel">
          ...
          <div style="flex:1;overflow-y:auto;">
            ...
            <div id="vp-paytable" style="font-size:0.78rem;"></div>
          </div>
          <div class="bp-bottom">
    ...
    function renderHand(cards, allowToggle) {
      els.hand.innerHTML = "";
      cards.forEach((card, i) => {
        const suitLetter = SUIT_LETTER[card.suit] || "S";
        const wrap = document.createElement("div"); wrap.className = "vp-card-wrap";
        wrap.innerHTML = `<img class="card-svg vp-card-img" src="/images/cards/${card.rank}${suitLetter}.svg" alt="${card.rank}${card.suit}" /><div class="vp-hold-badge${held[i]?" active":""} ">${held[i]?"HELD":"HOLD"}</div>`;
        if (allowToggle) {
          wrap.style.cursor = "pointer";
          wrap.addEventListener("click", () => { held[i] = !held[i]; renderHand(cards, true); });
        }
    ...
    els.deal.addEventListener("click", async () => {
      ...
      try {
        ...
        inRound = true;
        els.deal.classList.add("hidden"); els.draw.classList.remove("hidden"); els.amount.disabled = true;
    ...
    els.draw.addEventListener("click", async () => {
      if (busy || !inRound) return;
      busy = true; els.draw.disabled = true;
      try {
        const res = await Api.post("/games/videopoker/draw", { hold: held });
        inRound = false;
        els.draw.classList.add("hidden"); els.deal.classList.remove("hidden"); els.amount.disabled = false;
        ...
        els.result.textContent = isWin ? `🎉 ${name}! ${res.multiplier}x — paid ${UI.money(res.payout)}.` : `${name} — no win this hand.`;
        UI.applyAccountUpdate(accountState, res);
        UI.toast(isWin ? `Won ${UI.money(res.payout)} on Video Poker!` : "No win this hand.", isWin?"win":"info");
        held = [false,false,false,false,false];
      } catch (err) { UI.toast(err.message, "loss"); } finally { busy = false; els.draw.disabled = false; }
    });
